...
class GAGNN_V1_dipol(nn.Module):
    def __init__(self, output_dim=1, state_dim=64, num_message_passing_rounds=4, num_atom_types=5, num_rbf=20):
        super().__init__()
        # parameters
        ...
        self.atom_embedding_scalar = nn.Embedding(num_atom_types, state_dim) 
        self.atom_embedding_tri = nn.Embedding(num_atom_types, state_dim)  
        ...
        # Geometric product weights for message block 
        #self.weighted_gp_weights_M = nn.ParameterList([
            #nn.Parameter(torch.empty(20, state_dim).normal_(mean=0, std=1 / math.sqrt(8)))
            #for _ in range(num_message_passing_rounds)
        #])

        # Linear layer for geometric product for message block
        #self.gp_linear_M = nn.ModuleList([
    ...
    def forward(self, x):
        ...
        # Output layer 
        # Output layer (2 gated equivariant blocks)
...
class GAGNN_V1_scalarAndR2(nn.Module):
    def __init__(self, output_dim=1, state_dim=64, num_message_passing_rounds=4, num_atom_types=5, num_rbf=20):
        super().__init__()
        # parameters
        self.state_dim = state_dim
    ...
    def forward(self, x):
        ...
        # Output layer
        # -- R2 Prediction --
        # Scalar part of the multivector state
        #scalar_state = state[:, 0] 

        # Apply shared MLP to all atoms
        #atomwise_outputs = self.scalar_readout_mlp(scalar_state) 
        ...
        # -- scalar prediction --
        # Use scalar part of multivector state
        scalar_state = state[:, 0] 

        # Apply shared readout MLP to all scalar states
        atomwise_contributions = self.scalar_readout_mlp(scalar_state)  
